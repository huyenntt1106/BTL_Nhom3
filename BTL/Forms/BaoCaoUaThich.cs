...
namespace BTL.Forms
{
    public partial class frmBaoCaoUaThich : Form
    {
        public frmBaoCaoUaThich()
        {
            InitializeComponent();
        }

        private void frmBaoCaoUaThich_Load(object sender, EventArgs e)
        {
            btnIn.Enabled = false;
        ...
        private void btnHienThi_Click(object sender, EventArgs e)
        {
            ...
            sql1 += " and (c.NgayThue between '" + Functions.ConvertDateTime(mskTuNgay.Text) + "' and '" + Functions.ConvertDateTime(mskDenNgay.Text) + "') group by b.TenSach order by count(a.MaSach) desc";
            Class.Functions.RunSql(sql1);
            load_data1(sql1);


            sql2 += "and (d.NgayThue between '" + Functions.ConvertDateTime(mskTuNgay.Text) + "' and '" + Functions.ConvertDateTime(mskDenNgay.Text) + "') group by c.TenLoaiSach order by count(a.MaSach) desc";
            Class.Functions.RunSql(sql2);
            load_data2(sql2);


            sql3 += "and (d.NgayThue between  '" + Functions.ConvertDateTime(mskTuNgay.Text) + "' and '" + Functions.ConvertDateTime(mskDenNgay.Text) + "') group by c.TenTacGia order by count(a.MaSach) desc";
